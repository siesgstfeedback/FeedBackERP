...
const FeedbackForm = () => {
  ...
  const fetchStudentDetails = async () => {
    try {
      ...
      setStudent(data);
      fetchAllocations(data);      // You already have these two functions
      fetchFilledFeedbacks(data);  // working locally on the frontend
    } catch (err) {
  ...
  const fetchAllocations = async (studentData) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/form-fetch-allocations", {
        method: "POST",
      ...
      console.error("Error in fetchAllocations:", err);
  ...
  const fetchFacultyNames = async (allocationsData) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/form-faculty-names", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ empIds: facultyEmpIds }),
      });
  ...
  const fetchFilledFeedbacks = async (studentData) => {
    try {
      ...
      setCurrentFacultyIndex(data.length); // Update index based on filled feedbacks
    } catch (error) {
  ...
  const handleSubmit = async (e, allocation) => {
    ...
    try {
      ...
      if (currentFacultyIndex + 1 < allocations.length) {
        ...
        setCurrentFacultyIndex(allocations.length);
      }
    } catch (err) {
      console.error(err);
